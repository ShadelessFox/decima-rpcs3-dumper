...
import com.shade.decima.rpcs3.killzone3.rtti.RTTICompound;
...
public final class IdaExporter implements Exporter {
    ...
    private void idaStaticArray(RTTI rtti, Slice<?> slice, String memberType, String memberName) throws IOException {
        if (slice.length() > 0) {
            ...
            writer.newLine();
            writer.write("\tapply_type(%#08x, \"%s[%d]\");".formatted(address, memberType, slice.length()));
